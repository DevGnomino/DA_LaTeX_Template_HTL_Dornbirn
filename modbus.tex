...
Das Modbus Protokoll nur Layer 7 definiert, kann es in verschiedenen Übertragungssystemen implementiert werden:
\begin{itemize}
	...
	\item \textbf{Modbus Plus:} Funktioniert mittels Tokens, die von einem Gerät zum nächsten wandern. Jeweils das Gerät, das den Token hat, ist der Client. (https://www.rtautomation.com/rtas-blog/good-bye-modbus-plus/)
\end{itemize}
(hier vielleicht modbus bild einfügen: MODBUS communication stack)
...
Serielle Kommunikation bedeutet grundsätzlich, dass eine Nachricht Bit für Bit (seriell) versendet wird. Dafür gibt es verschiedene Standards, die unterschiedliche Kriterien aufweisen (siehe Tabelle \ref{tab:serielle}) (https://www.elektronik-kompendium.de/sites/com/2501161.htm)
\begin{table}[H]
	\caption{Serielle Kommunikations Standards \label{tab:serielle}}
	...
	\end{tabularx}
\end{table}
https://ipc2u.de/artikel/wissenswertes/die-hauptunterschiede-zwischen-rs-232-rs-422-und-rs-485/

In den Lüftungsgeräte der Firma Bösch wird die RS485 Schnittstelle verwendet.

\paragraph{RS485} \label{rs485}
RS485 ist ein Standard, welcher die elektrischen bzw. physikalischen Eigenschaften einer Schnittstelle definiert. Es befindet sich auf Schicht 1 des ISO/OSI Modells. Der Standard definiert nicht die Struktur der zu übertragenen Daten. Daher kann es in Kombination mit den unterschiedlichsten Protokollen, auf den darüber liegenden Schichten verwendet werden (z.B. Modbus RTU) (https://www.janitza.de/service-support/kommunikation-ueber-die-rs485-schnittstelle.html).

RS485 verwendet für das Übertragen 2 Leitungen. Es gibt eine Leitung, auf der das  Datenbit übertragen wird. Wenn das Datenbit 1 ist, ist ein High Spannungspegel auf dieser Leitung, wenn das Datenbit 0 ein Low Spannungspegel. Auf der zweiten Leitung wird das invertierte Datenbit übertragen. Wenn das Datenbit 1 ist, ist ein Low Spannungspegel auf dieser Leitung, wenn das Datenbit 0 ein High Spannungspegel. Der Spannungsunterschied zwischen den beiden Leitungen muss beim Übertragen mindestens 1,5V betragen. (https://www.ti.com/lit/an/slla272d/slla272d.pdf?ts=1708495351014) \newline Dadurch, dass es 2 Leitungen gibt und die Spannungsunterschiede genügend groß definiert sind, ist RS485 ziemlich Resistent gegen elektromagnetische Störungen. Die Daten können auch über längere Strecken sicher übertragen werden. (https://www.ti.com/lit/an/slla272d/slla272d.pdf?ts=1708495351014, https://heinen-elektronik.de/glossar/rs485-schnittstelle/).

Wie wird das nächste Bit signalisiert, wenn beide Bits gleich sind - der Pegel bleibt ja gleich (wird Polarität geswitched?).

RS485 kann entweder im Halb- oder Vollduplex Modus aufgesetzt werden. \newline Halbduplex bedeutet, dass entweder gesendet oder empfangen werden kann, aber nicht beides gleichzeitig. Der Bus benötigt nur 1 Leitungspaar (2 Leitungen). \newline Vollduplex bedeutet, dass Senden und Empfangen parallel funktioniert. D.h. es gibt 2 Leitungspaare (4 Leitungen), eine für das Senden, eine für das Empfangen. (https://www.ti.com/lit/an/slla272d/slla272d.pdf?ts=1708495351014)

Noch nicht verwendet: (https://www.eltima.com/de/article/rs485-communication-guide/)

Eine wichtige Information, die uns durch unseren Firmenbetreuer mitgeteilt wurde, ist, dass der Bus nach dem letzten Teilnehmer einen Terminierungswiderstand benötigt. Dadurch lässt sich eine Reflexion des Signals verhindern. (vgl. https://www.ti.com/lit/an/slla272d/slla272d.pdf?ts=1708495351014). Auf den QBM Geräten gibt es dafür einen Schalter, mit diesem man den Terminierungswiderstand ein bzw. ausschalten kann. Das Vergessen der Existenz dieses Schalters hat beim Test diverse male zu scheinbar unerklärlichen Fehlern geführt.

\paragraph{Baudrate} \label{baud_rate}
%\begin{quotation}
%	\emph{
%		Die Baudrate ist die Häufigkeit pro Sekunde, mit der ein serielles Kommunikationssignal Zustände ändert; ein Zustand ist entweder ein Spannungspegel, eine Frequenz oder ein Frequenzphasenwinkel.}
%\end{quotation}
%https://www.ibm.com/docs/de/aix/7.3?topic=parameters-baud-rate
...
!!!!!In dieser Diplomarbeit wird dennoch der Begriff Baudrate verwendet, obwohl die in den Konfigurationsdateien angegebenen Datenraten in Bit/s gemessen werden. (In ebm papst doku steht Übertragungsrate --> vlt in config files baudrate in Datenrate umbenennen? - in qbm doku heißt es baudrate, wird aber in bit/s angegeben)!!!!
https://www.kunbus.com/de/baudraten-grundlagen
Gängige Datenraten sind für Modbus 9600 Bit/s und 19200 Bit/s. In den Lüftungsgeräten der Firma Bösch wird 19200 Bit/s verwendet.
