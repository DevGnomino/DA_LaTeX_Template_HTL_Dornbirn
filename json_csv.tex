Lüftungsgeräte beinhalten unterschiedlichste Komponenten, beispielsweise Ventilatoren, Luftdrucksensoren oder Temperatursensoren, mit denen die Funktion überwacht und gesteuert wird. Weil die Lüftungsgeräte auf Kundenanforderungen angepasst werden, ist die Anordnung und Menge der verbauten Komponenten in den meisten Fällen unterschiedlich. Daher ist es wichtig, dass die \acs{rltanzeige} parametrierbar ausgeführt werden kann. Um dies zu erzielen, wurde vom Projektauftraggeber die Idee von einer Konfigurationsdatei vorgeschlagen. Mit dieser kann von einem  Servicetechnikerinnen und Servicetechnikern bei der Installation des Lüftungsgeräts die Konfiguration an die \acs{rltanzeige} übergeben werden. So können für das jeweilige Lüftungsgerät immer die richtigen Parameter angezeigt werden.
...
\acs{csv} bringt einige Vorteile mit sich. 
in Excel editierbar
einfach zu lesen
von vielen Programmiersprachen unterstützt


Ein weiterer Vorteil von \acs{json} mittlerweile an Kompatibilität kaum zu übertreffen. Auch der Trend scheint immer mehr gegen \acs{json} zu gehen. Flexibilität und die Strukturiertheit


Schließlich ist die Möglichkeit der Verschachtelung mit \acs{json} der guten Leserlichkeit von \acs{csv} im benötigten Anwendungsbereich überlegen. Dies war der ausschlaggebende Grund warum sich für das \acs{json} Datenformat entschieden wurde.
